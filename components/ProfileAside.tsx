...
import ImageUserCard from "./cards/ImageUserCard";
...
function ProfileAside() {
  ...
  return (
    <aside
      className={`flex-none ${
        isSideBarOpen ? "basis-full md:basis-1/3" : "basis-0 "
      } overflow-hidden  border-neutral-800 md:border-l-2 transition-all duration-600 h-screen sticky top-0  bg-[#141414]`}
    >
      {/* Icon Bar */}
      <div className="flex flex-col items-center">
        <div className="relative flex flex-col h-screen  w-[90%]">
          <div className="mt-4 flex flex-row justify-between">
            <button
              className="flex justify-center items-center h-[30px] w-[30px] rounded-full bg-neutral-600"
              onClick={() => {
                const open:any = false;
                dispatch(utilBools.setIsSideBarOpen(open))}}
            >
          ...
          <div className="flex flex-col items-center justify-center flex-none w-full mt-4">
            ...
            <p className="text-base md:text-xs text-neutral-400 mt-0.5">{userState?.email}</p>

            <button className="mt-4 text-lg md:text-sm py-1.5 px-5 bg-neutral-600 text-white  rounded-full">
              Edit
            </button>
          </div>
          ...
          <button
            onClick={() => {
              ...
              dispatch(utilBools.setPostEditData(edit));
            }}
            className="mt-8 w-full flex justify-stretch  flex-row justify-center py-5 md:py-3 text-xl text-white md:text-base rounded-2xl bg-blue-600"
          >
            Create Post
          </button>

          <div className="grid grid-cols-2 gap-x-2 gap-y-2 mt-8 mb-8 overflow-y-auto scrollbar-hide">
            {userPostsState?.map((post) => (
              <ImageUserCard key={post.id} post={post} />
            ))}
          </div>
        </div>
      </div>
